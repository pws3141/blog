...
These C-statistics seem to be very low (most under $0.5$). There are some
reasons why this might be the case (that I should probably have a look at at some point):

1.	Stepwise selection is likely overfitting to noise in the training set.
2.	The predictors in `lung` may not be strongly associated with survival.
3.	Some predictors might violate proportional hazards, hurting model performance.
4.	Small test set size reduces the reliability of C-index estimates.
