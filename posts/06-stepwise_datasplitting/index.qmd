...
subtitle: "Some critiques of data-splitting and stepwise"
author:
  - name: Paul Smith
date: "2025-03-05"
categories: [code, r, statistics, model fitting, model building]
format:
  html:
    code-fold: false
execute:
...
Currently, at work we use a stepwise procedure to chose explanatory variables in the model. This stepwise procedure is sometimes a mix of clinical judgement with some form of automated selection, and sometimes it is fully automated. This is often done on a *test* set which consists of around $70\%$ of the full data. For an example of a fully automated stepwise procedure on a $70\%$ test set, see @collett2017factors.

There are lots of issues that result from this process: from the use of stepwise, and from not using all the data to fit the model (*data-splitting*).

# Issues with the stepwise procedure

There are lots of issues from using a stepwise procedure. Some of these are related to the procedure itself, and some related to the ability of the statistician to not have to think about the problem they are trying to solve. From @harrell2001regression [Section 4.3],
...
To prevent regression coefficients being too large, Tibshirani's^[of *An Introduction to Statistical Learning* fame.] *lasso* procedure can be used [@tibshirani1996regression]. This shrinks coefficient estimates towards zero by including a constraint that the sum of absolute values of the coefficient estimates must be less than some $k$.

@derksen1992backward found that when the stepwise procedure was used, the final model represented noise between $20\%$ and $74\%$ of the time, and the final model contained less than halve of the actual number of authentic predictors. To see this in action, Frank Harrell has some simulations on his 'Regression Modelling Strategies' [course notes](https://hbiostat.org/rmsc/multivar#sec-multivar-variable-selection). 
